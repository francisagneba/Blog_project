...
class ArticleController extends AbstractController
{
    ...
    /**
     * @Route("/", name="app_article_index_dashboard", methods={"GET"})
     */
    ...
    public function new(Request $request, ArticleRepository $articleRepository): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            //Date insertion nouvelle article.
            $article->setCreatedAt(new \DateTimeImmutable());
            //Pour la récuperation de l'image
            $file = $form["imageFile"]->getData();
            // On va creer un service qui nous permettra de renomé nos images 
            $file_url = $this->uploadFile->saveFile($file);
            // On va sauvegarder le nom de l'image à l'interieur de Article
             $article->setImageUrl($file_url);

            //On va sauvegarder l'auteur de l'article
